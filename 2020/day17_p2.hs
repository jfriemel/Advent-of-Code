...
type PocketDimension = [(Int, Int, Int, Int)]
...
parseDimension inp = [(x,y,1,1) | y <- [0 .. length inp - 1], x <- [0 .. length (inp !! y) - 1], (inp !! y) !! x == '#']
...
cycleOnce :: [((Int, Int, Int, Int), Int)] -> PocketDimension -> PocketDimension
cycleOnce []     dim = []
cycleOnce ((p,c):cs) dim
  | c == 3 || p `elem` dim && c == 2 = p : cycleOnce cs dim
  | otherwise                        =     cycleOnce cs dim

activeNeighborCounts :: PocketDimension -> [((Int, Int, Int, Int), Int)]
activeNeighborCounts dim = toList (fromListWith (+) [(coords, 1) | coords <- concatMap (getNeighbors dim) dim])

getNeighbors :: PocketDimension -> (Int, Int, Int, Int) -> PocketDimension
getNeighbors dim (x,y,z,w) = [(i,j,k,l) | i <- [x-1,x,x+1],
                                          j <- [y-1,y,y+1],
                                          k <- [z-1,z,z+1],
                                          l <- [w-1,w,w+1],
                                          i /= x || j /= y || k /= z || l /= w]